...
// Each field is tagged 'all' | 'expedition' | 'exploration' so the modal filter tabs can narrow the view
const FIELD_DEFS = [
  ['兵種', h => h.hero_class, 'all'],
  ['レアリティ', h => h.rarity, 'all'],
  ['世代', h => '第' + h.generation + '世代', 'all'],
  ['総合ティア', h => h.tier_overall, 'all'],
  ['遠征ティア', h => h.tier_expedition, 'expedition'],
  ['探検ティア', h => h.tier_exploration, 'exploration'],
  ['入手方法', h => h.how_to_obtain || '-', 'all'],
  ['解説', h => h.notes || '-', 'all'],
  ['推奨用途', h => h.best_use || '-', 'all'],
...
function renderCompareContent() {
  ...
  const rows = FIELD_DEFS.filter(([, , type]) => compareFilterMode === 'all' || type === 'all' || type === compareFilterMode);
