...
const classColors = { Infantry: '#E74C3C', Marksman: '#3498DB', Lancer: '#2ECC71' };
const rarityColors = raw.rarity_colors || { Rare: '#3498DB', Epic: '#9B59B6', Legendary: '#F1C40F' };
...
function populateGenFilter() {
  ...
  gens.forEach(g => {
    ...
    opt.value = g; opt.textContent = 'Gen ' + g;
    sel.appendChild(opt);
  });
}
...
function render() {
  ...
  list.forEach(h => {
    ...
    card.innerHTML = `
      ...
      <div class="meta">Gen ${h.generation} ・ ${h.rarity}</div>
...
function openCompareModal() {
  if (selected.size < 2) { alert('比較するヒーローを2体以上選んでください'); return; }
  const list = heroes.filter(h => selected.has(h.name));
  const rows = [
    ['クラス', h => h.hero_class],
    ['レアリティ', h => h.rarity],
    ['世代', h => 'Gen ' + h.generation],
    ['総合ティア', h => h.tier_overall],
    ['遠征(Expedition)ティア', h => h.tier_expedition],
    ['探索(Exploration)ティア', h => h.tier_exploration],
